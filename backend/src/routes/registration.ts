...
router.post('/approve', asyncHandler(async (req: Request<{}, {}, { license_number: string }>, res: Response) => {
    ...
    const { rows: registrations } = await pool.query( //fetch based on license number
      `SELECT school_email, 
    ...
    const { rows: existingDrivers } = await pool.query( //checks if license in driver === license in registration
      `SELECT id 
      FROM drivers 
      WHERE license_number = $1`,
      [license_number]
